            ...
            if (client.available())                   // I dati inviati dal client sono disponibili
            {
                ...
                if (c == '\n' && currentLineIsBlank)
                {
                   ...
                   if (StrContains(HTTP_req, "ajax_inputs"))
                   {
                      ...
                      client.println("HTTP/1.1 200 OK");             // risposta standard HTTP: OK!
                    ...
                    {
                      //    ... se è una richiesta della home page, invia la pagina web (file index.htm)
                      ...
                      //    (la verifica viene fatta solo sulla home directory)
                      //    ... se è una richiesta del css, invia il file relativo
                      else if (StrContains(HTTP_req, "GET /style.css"))
                      {
                         client.println("HTTP/1.1 200 OK");             // risposta standard HTTP: OK!
                         client.println("Content-Type: text/css");      // Invia la parte rimanente di header
                         client.println("Connection: keep-alive");
                         client.println();
                         webFile = SD.open("style.css");                 // apre il file caa
                      }
                      //    (la verifica viene fatta solo sulla home directory)
                      //    ... se è una richiesta del javascript, invia il file relativo
                      else if (StrContains(HTTP_req, "GET /ajax.js"))
                      {
                         client.println("HTTP/1.1 200 OK");             // risposta standard HTTP: OK!
                         client.println("Content-Type: text/javascript");      // Invia la parte rimanente di header
                         client.println("Connection: keep-alive");
                         client.println();
                         webFile = SD.open("ajax.js");                   // apre il file javascript
                      }
                      //    ... se è una richiesta di una immagine, invia il file relativo
                      //    (la verifica viene fatta solo sulla home directory)
                      else if (strstr(HTTP_req, "GET /lamp.png"))
                      {
                         client.println("HTTP/1.1 200 OK");             // risposta standard HTTP: OK!
                         client.println("Connection: keep-alive");
                         client.println();
                         webFile = SD.open("lamp.png");                  // apre il file immagine
                      }
                      else if (strstr(HTTP_req, "GET /sensor.png"))
                      {
                         client.println("HTTP/1.1 200 OK");             // risposta standard HTTP: OK!
                         client.println("Connection: keep-alive");
                         client.println();
                         webFile = SD.open("sensor.png");                // apre il file immagine
                      }
                      if (webFile)
                      {
                         Serial.println(HTTP_req);
